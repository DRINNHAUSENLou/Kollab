    ...
    <svg class="h-9 w-9 text-purple-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
      <path d="M320 64C302.3 64 288 78.3 288 96L288 99.2C215 114 160 178.6 160 256L160 277.7C160 325.8 143.6 372.5 113.6 410.1L103.8 422.3C98.7 428.6 96 436.4 96 444.5C96 464.1 111.9 480 131.5 480L508.4 480C528 480 543.9 464.1 543.9 444.5C543.9 436.4 541.2 428.6 536.1 422.3L526.3 410.1C496.4 372.5 480 325.8 480 277.7L480 256C480 178.6 425 114 352 99.2L352 96C352 78.3 337.7 64 320 64zM258 528C265.1 555.6 290.2 576 320 576C349.8 576 374.9 555.6 382 528L258 528z"/>
            ...
            <button id="profileBtn" class="flex items-center gap-3 px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-700 hover:opacity-80 text-white rounded-lg shadow-md transition-all duration-300">
                <img src="{{ Auth::user()->profile_picture ?? '/images/default-avatar.png' }}" class="w-8 h-8 rounded-full border-2 border-white" alt="Avatar" />
            ...
            <div id="profileDropdown" class="hidden absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden z-50">
                ...
                <a href="" class="block px-4 py-2 text-gray-700 hover:bg-purple-50 transition">Profil</a>
...
<div class="flex flex-1">
    ...
    <nav class="flex-1 mt-4 space-y-4">
        ...
        <!-- Tableau de bord -->
        <a href="{{ route('projet.show', $projet->id_projet) }}" class="flex items-center px-4 py-2 hover:bg-purple-700">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="w-10 h-10 text-purple-300 flex-shrink-0">
        ...
        <!-- Roadmap -->
        <a href="{{ route('projet.sprints.index', $projet->id_projet) }}" class="flex items-center px-4 py-2 hover:bg-purple-700">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="w-10 h-10 text-purple-300 flex-shrink-0">
...
<div class="container mt-4">
    <h2>Epics du projet : <strong>{{ $projet->nom }}</strong></h2>

    <div class="mb-3">
        <a href="{{ route('epic.create', $projet->id_projet) }}" class="btn btn-primary">
            + Ajouter un Epic
        </a>
        <a href="{{ route('projet.show', $projet->id_projet) }}" class="btn btn-secondary">
    ...
    @else
        <table class="table table-striped">
            ...
            <tbody>
                @foreach ($projet->epics as $epic)
                    <tr>
                        <td>{{ $epic->titre }}</td>
                        <td>{{ ucfirst($epic->priorite) }}</td>
                        <td>{{ $epic->description ?? '—' }}</td>
                        <td>
                            <a href="{{ route('epic.edit', [$projet->id_projet, $epic->id_epic]) }}" class="btn btn-sm btn-warning">Modifier</a>
                            <form action="{{ route('epic.delete', [$projet->id_projet, $epic->id_epic]) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-danger" onclick="return confirm('Supprimer cet epic ?')">Supprimer</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
